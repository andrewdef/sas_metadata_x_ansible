/** 
* Same as fsutil_dirtree_walk, but the callback does not have to return a value, so it can also use
* non-macro statements (eg. data steps, procs)
*
...
%macro fsutil_dirtree_walk_nores(root, dirname_regexp=, filename_regexp=, callback=walk_callback, maxdepth=1,arg=);
	...
	%fsutil_dirtree_walk_nores_p(%superq(root), %superq(root), 1, &id, &dir_regexp_id., &filename_regexp_id., maxdepth=&maxdepth, callback=&callback,arg=&arg);
%mend;
...
%macro fsutil_dirtree_walk_nores_p(source, root, level, parentId, dir_regexp_id, filename_regexp_id, maxdepth=, callback=, arg=);
	...
	%do i = 1 %to &dnum;
		...
		%if &isDirectory %then %do;
			...
			%end;
			%else %do;
				%fsutil_dirtree_walk_nores_p(&source., &root.&path_sep.&memname., %eval(&level+1), &id, &dir_regexp_id., &filename_regexp_id., maxdepth=&maxdepth, callback=&callback,arg=&arg);
			%end;
			
			%if &callback ne %then %do;
				%&callback(id=&id, type=D, memname=&memname., level=&level, parent=&parentId, context=&root., arg=&arg);
			%end;
			%else %do;
		...
		%else %do;
			...
			%if &callback ne %then %do;
				%&callback(id=., type=F, memname=&memname., level=&level, parent=&parentId, context=&root.,arg=&arg);
			%end;
			%else %do;
				%put parent=&parentId, member=&memname, level=&level, type=F;
			%end;
			%if &SYSCC. > 4 %then %goto theend;
		%end;
...
%mend fsutil_dirtree_walk_nores_p;
