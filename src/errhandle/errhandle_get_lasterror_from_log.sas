/** 
* Returns the last ERROR message by parsing the SAS log file specified as input parameter. If
...
%macro errhandle_get_lasterror_from_log(return_var, log_file=);
	%if %superq(log_file) = %then
		%errhandle_get_current_logfile(log_file);
		
	%errhandle_get_error_from_logfile(%superq(log_file), &return_var., error_type=ERROR, first_or_last=LAST);

%mend errhandle_get_lasterror_from_log;
